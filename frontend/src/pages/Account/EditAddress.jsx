...
export const EditAddress = () => {
    ...
    const onFormChange = (e) => {
        if (e.city) {
            ...
            setTown([]);
            setNeighbourhood([]);
            form.setFieldsValue({ "town": "", "district": "", "neighbourhood": "", })
    ...
    useEffect(() => {
        if (fetchedAddress.error) {
            message.error(fetchedAddress.error.message);
        }
    ...
    return (
        <Spin spinning={fetchedAddress.isLoading}>
            <Form
                name='basic'
                labelCol={{
                    span: 8,
            ...
            >
                ...
                <div className="button__wrapper my-5">
                    <button htmlType="submit" className={`text-uppercase button btn-primary w-100 ${isLoading ? "disabled" : ""}`} style={{ padding: "20px " }} >
                        Submit
                    </button>
                </div>
            </Form>
